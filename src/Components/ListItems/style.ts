...
export const Wrapper = styled(FlexCol)`
  ...
  border-top: 1px solid
    ${(props: any) => (props.border ? props.theme.bg.border : 'transparent')};
  ...
  &:hover > div > div h3,
  &:hover .action {
    color: ${(props: any) => (props.clickable ? props.theme.brand.alt : '')};
  }
`;
...
export const Meta = styled(H4)`
  ...
  ${(props: any) => (props.nowrap ? Truncate() : '')};
`;
...
export const StyledCard = styled.div`
  flex-direction: column;
  display: ${(props: any) => (props.smallOnly ? 'none' : 'flex')};

  @media (max-width: 768px) {
    display: ${(props: any) => (props.largeOnly ? 'none' : 'flex')};
  }
`;
...
export const ListHeader = styled(FlexRow)`
  ...
  margin-top: ${(props: any) => (props.secondary ? '24px' : '0')};
`;

...
export const Item = styled(FlexCol)`
  padding: 16px;
  flex: 0 0 280px;
  flex-order: ${(props: any) => (props.active ? '2' : '1')};
  background-color: ${({ theme }) => theme.bg.default};
  color: ${({ theme }) => theme.text.default};
  border-radius: 16px;
  margin-right: 24px;
  justify-content: space-between;
  position: relative;
  opacity: ${(props: any) => (props.active ? '0.85' : '1')};
...
export const ButtonContainer = styled(FlexRow)`
  justify-content: ${(props: any) => (props.center ? 'center' : 'flex-end')};
  align-items: center;
...
export const SearchResult = styled.li`
  ...
  border-bottom: 2px solid
    ${(props: any) => (props.focused ? 'transparent' : props.theme.bg.border)};

  &:hover {
    background: ${(props: any) => props.theme.brand.alt};
    cursor: pointer;

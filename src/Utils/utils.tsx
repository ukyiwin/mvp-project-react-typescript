...
export const draftOnlyContainsEmoji = (raw: any) =>
  raw.blocks.length === 1 &&
 ...
 */
export const encode = (strg: string) =>
  Buffer.from(strg).toString('base64');
...
export const debounce = (func: any, wait: number, immediate: boolean) => {
  let timeout;
  return function() {
    // tslint:disable-next-line:one-variable-per-declaration no-this-assignment
    const context = this,
      args = arguments;
    const later = () => {
      timeout = null;
...
export const getLinkPreviewFromUrl = (url: string) =>
  fetch(`https://links.unizonn.com/?url=${url}`)
    .then((response) => {
...
export const truncateNumber = (num: number, places: number = 1) => {
  const truncated =
  num > 999 ? (num / 1000).toFixed(places) + 'k' : num;

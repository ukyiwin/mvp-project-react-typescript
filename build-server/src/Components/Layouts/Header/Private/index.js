"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const React = require("react");
// import UIkit from 'uikit/src/js/uikit';
const react_router_dom_1 = require("react-router-dom");
const SearchContainer_1 = require("Components/SearchContainer");
// tslint:disable-next-line:max-line-length
const maleP = 'data:image/svg+xml;utf8;base64,';
// tslint:disable-next-line:no-any
const PrivateHeader = (props) => {
    return (React.createElement("div", { "data-uk-sticky": "sel-target: .uk-navbar-container; cls-active: uk-navbar-sticky" },
        React.createElement("nav", { className: "uk-navbar-container uk-margin uk-background-secondary raised", "uk-navbar": true, style: { elevation: 5, borderBottomWidth: 1, borderBottomColor: '#212121' } },
            React.createElement("div", { className: "nav-overlay uk-navbar-left" },
                React.createElement("button", { className: "uk-navbar-toggle nav-overlay uk-hidden@m", type: "button", "uk-navbar-toggle-icon": true, "uk-toggle": "target: #offcanvas-nav", "uk-icon": "icon: menu; ratio: 1.5" }),
                React.createElement(react_router_dom_1.Link, { to: "/", className: "uk-navbar-item uk-logo", href: "#" },
                    React.createElement("img", { src: require('../../../../Assets/main/logof.png'), style: { height: 70 }, "uk-svg": true })),
                React.createElement("ul", { className: "nav-overlay uk-navbar-nav uk-visible@m" },
                    React.createElement("li", null,
                        React.createElement(react_router_dom_1.Link, { to: "/home", className: "uk-light" }, "Articles")),
                    React.createElement("li", null,
                        React.createElement(react_router_dom_1.Link, { to: "/forum", className: "uk-light" }, "Forums")),
                    React.createElement("li", null,
                        React.createElement(react_router_dom_1.Link, { to: "/library", className: "uk-light" }, "Library")),
                    React.createElement("li", null,
                        React.createElement(react_router_dom_1.Link, { to: "/home", className: "uk-light" }, "Connections"))),
                React.createElement("div", { className: "nav-overlay uk-navbar-right uk-visible@s" },
                    React.createElement("div", { className: "main-searchbar" },
                        React.createElement(SearchContainer_1.default, null)),
                    props.isAuthenticated ? React.createElement(React.Fragment, null,
                        React.createElement("ul", { className: "uk-iconnav" },
                            React.createElement("li", { className: "uk-animation-toggle" },
                                React.createElement(react_router_dom_1.Link, { className: "uk-animation-shake", to: "/message", "uk-icon": "icon: calendar; ratio: 1.5" })),
                            React.createElement("li", { className: "uk-animation-toggle" },
                                React.createElement("button", { className: "uk-animation-shake", "uk-icon": "icon: bell; ratio: 1.5" }))),
                        React.createElement("ul", { className: "uk-navbar-nav" },
                            React.createElement("li", null,
                                React.createElement("a", { href: "#" },
                                    React.createElement("div", { className: "uk-animation-toggle" },
                                        React.createElement("img", { className: "uk-border-circle uk-animation-kenburns uk-animation-fast", src: props.avatar ? props.avatar : maleP, width: "40", height: "40", alt: "img" }))),
                                React.createElement("div", { "data-uk-dropdown": "mode: click" },
                                    React.createElement("ul", { className: "uk-nav uk-dropdown-nav" },
                                        React.createElement("li", null,
                                            React.createElement(react_router_dom_1.Link, { to: "/profile", className: "uk-text-bold" }, "Profile")),
                                        React.createElement("li", null,
                                            React.createElement(react_router_dom_1.Link, { to: "/message", className: "uk-text-bold" }, "Reading Score")),
                                        React.createElement("li", null,
                                            React.createElement(react_router_dom_1.Link, { to: "/message", className: "uk-text-bold" }, "Privacy")),
                                        React.createElement("li", { className: "uk-nav-divider" }),
                                        React.createElement("li", null,
                                            React.createElement(react_router_dom_1.Link, { to: "/message", className: "uk-text-bold" }, "Support")),
                                        React.createElement("li", null,
                                            React.createElement("a", { onClick: props.logout, className: "uk-text-bold" }, "Logout")))))),
                        React.createElement("div", { className: "uk-navbar-nav uk-padding-small" },
                            React.createElement(react_router_dom_1.Link, { to: "/write", className: "uk-button uk-button-primary uk-button-small" }, "Write")),
                        " ") :
                        React.createElement("div", { className: "uk-navbar-nav uk-padding-small" },
                            React.createElement(react_router_dom_1.Link, { to: "/login", className: "uk-button uk-button-default uk-dark uk-button-small" }, "Get started"))),
                React.createElement("div", { className: "nav-overlay uk-navbar-right uk-hidden@s" }, props.isAuthenticated ? React.createElement(React.Fragment, null,
                    React.createElement("ul", { className: "uk-iconnav" },
                        React.createElement("li", { className: "uk-animation-toggle" },
                            React.createElement(react_router_dom_1.Link, { className: "uk-animation-shake", to: "/message", "uk-icon": "icon: bell; ratio: 1.5" })),
                        React.createElement("div", { "data-uk-drop": "mode: click", className: "uk-width-large uk-width-1-1@s" },
                            React.createElement("div", { className: "uk-card uk-card-body uk-card-default" },
                                React.createElement("li", { className: "unread-notification" },
                                    React.createElement("a", null,
                                        React.createElement("span", null),
                                        "fgfggfg",
                                        React.createElement("div", { className: "notification-metadata" },
                                            "ghghghh",
                                            React.createElement("br", null),
                                            React.createElement("small", null, '20/2/2018'))))))),
                    React.createElement("ul", { className: "uk-navbar-nav" },
                        React.createElement("li", null,
                            React.createElement("a", { href: "#" },
                                React.createElement("div", { className: "uk-animation-toggle" },
                                    React.createElement("img", { className: "uk-border-circle uk-animation-kenburns uk-animation-fast", src: props.avatar ? props.avatar : maleP, width: "40", height: "40", alt: "img" }))),
                            React.createElement("div", { "data-uk-dropdown": "mode: click" },
                                React.createElement("ul", { className: "uk-nav uk-dropdown-nav" },
                                    React.createElement("li", null,
                                        React.createElement(react_router_dom_1.Link, { to: "/profile", className: "uk-text-bold" }, "Profile")),
                                    React.createElement("li", null,
                                        React.createElement(react_router_dom_1.Link, { to: "/message", className: "uk-text-bold" }, "Reading Score")),
                                    React.createElement("li", null,
                                        React.createElement(react_router_dom_1.Link, { to: "/message", className: "uk-text-bold" }, "Privacy")),
                                    React.createElement("li", { className: "uk-nav-divider" }),
                                    React.createElement("li", null,
                                        React.createElement(react_router_dom_1.Link, { to: "/message", className: "uk-text-bold" }, "Support")),
                                    React.createElement("li", null,
                                        React.createElement("a", { onClick: props.logout, className: "uk-text-bold" }, "Logout")))))),
                    React.createElement("div", { className: "uk-navbar-nav uk-padding-small" },
                        React.createElement(react_router_dom_1.Link, { to: "/write", className: "uk-button uk-button-primary uk-button-small" }, "Write")),
                    React.createElement("div", { className: "nav-overlay uk-navbar-right" },
                        React.createElement("a", { className: "uk-navbar-toggle", "uk-search-icon": "", "uk-toggle": "target: .nav-overlay; animation: uk-animation-fade", href: "#" })),
                    React.createElement("div", { className: "uk-flex-1", 
                        /* tslint:disable-next-line:jsx-boolean-value */
                        hidden: true },
                        React.createElement("div", { className: "uk-navbar-item uk-width-expand" },
                            React.createElement(SearchContainer_1.default, null)),
                        React.createElement("a", { className: "uk-navbar-toggle", "data-uk-close": "ratio: 1", "data-uk-toggle": "target: .nav-overlay; animation: uk-animation-fade", href: "#" })))
                    :
                        React.createElement("div", { className: "uk-navbar-nav uk-padding-small" },
                            React.createElement(react_router_dom_1.Link, { to: "/login", className: "uk-button uk-button-default uk-dark uk-button-small uk-border-rounded" }, "Login")))))));
};
exports.default = PrivateHeader;
//# sourceMappingURL=index.js.map
...
export const LinkPreviewContainer = styled.a`
  display: flex;
  flex-direction: ${(props: any) => (props.size === 'large' ? 'row' : 'row')};
  align-items: ${(props: any) => (props.size === 'large' ? 'flex-start' : 'center')};
  border-radius: 4px;
...
export const LinkPreviewImage = styled.div`
  overflow: hidden;
  min-width: ${(props: any) => (props.size === 'large' ? '100%' : '64px')};
  min-height: 64px;
...
export const LinkPreviewSkeleton = styled.div`
  ...
  margin: ${(props: any) => (props.margin ? props.margin : '16px')};
`;

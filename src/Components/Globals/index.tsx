...
export const zIndex = new function() {
  ...
  const hidden = base - 2; // this content should be hidden completely (USE ADD'L MEANS OF HIDING)
  ...
  if(process.browser){
    slider = window.innerWidth < 768 ? chrome + 1 : chrome;
  }

  const composer = slider - 2; // composer should never appear above the slider
...
export const Spinner = styled.span`
  ...
  &:before {
    ...
    position: ${(props: any) => (props.inline ? 'relative' : 'absolute')};
    top: ${(props: any) => (props.inline ? '0' : '50%')};
    left: ${(props: any) => (props.inline ? '0' : '50%')};
    width: ${(props: any) => (props.size !== undefined ? `${props.size}px` : '16px')};
    height: ${(props: any) => (props.size !== undefined ? `${props.size}px` : '16px')};
    margin-top: ${(props: any) =>
      props.size !== undefined ? `-${props.size / 2}px` : '-8px'};
    margin-left: ${(props: any) =>
      props.size !== undefined ? `-${props.size / 2}px` : '-8px'};
    border-radius: 50%;
    border: ${(props: any) => '2px'} solid
      ${(props: any) =>
        props.color
